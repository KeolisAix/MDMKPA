...
// END
if($R == 1){
	$sql="SELECT id,comment from \"aix_\".time_tables WHERE id=(SELECT time_table_id FROM \"aix_\".time_table_dates WHERE date='".$Date."')";
	$req=pg_query($sql);
    $nb=pg_num_rows($req);	
...
if($R == 4){
    if($ExDay == "0"){
        ...
        $nbcal = "";
            ...
            $nbcal = "1";            
            $reqP = "SELECT DISTINCT R.\"name\", R.\"id\" FROM aix_.routes r, aix_.time_tables_vehicle_journeys t1, aix_.vehicle_journeys v1, aix_.lines l, aix_.time_tables t2 WHERE ((((t1.vehicle_journey_id = v1.id) AND (v1.route_id = r.id)) AND (r.line_id = l.id)) AND (t2.id = t1.time_table_id)) AND L.name='".$ligne."' AND t2.id=".$Cal." ORDER BY R.\"id\" ASC";
            $sqlP=pg_query($reqP);
